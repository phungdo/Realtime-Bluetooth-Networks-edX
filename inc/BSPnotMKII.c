...
// but it is meant to stand alone in case the MKII BoosterPack is
// unavailable.  Some features are simply not available.  Others are
// implemented using the hardware on the LaunchPad or are completely
// re-designed.
...
// What is different?
// Buttons:
//   Use the two buttons on the LaunchPad.
// Joystick:
//   Continue measuring analog on PB5/AIN11 and PD3/AIN4 and digital
//   on PE4.  The user may connect external potentiometers and a
//   switch for the educational value, but the software will continue
//   to work regardless.  Wire the switch to be zero when pressed.
// RGB LED:
//   This is not implemented.  There is a 3-color LED on the
//   LaunchPad, but PF1 (LED red) is already used for Profile 3, and
//   PF2 (LED blue) is already used for the buzzer.
// Buzzer:
//   Continue to output a square wave on PF2.  The user may connect
//   external headphones through a 1k resistor for the educational
//   value, but the software will continue to work regardless.
// Accelerometer:
//   Continue measuring analog on PD0/AIN7, PD1/AIN6, and PD2/AIN5.
//   The user may connect external potentiometers for the educational
//   value, but the software will continue to work regardless.
// Microphone:
//   Continue measuring analog on PE5/AIN8.  The user may connect
//   external potentiometers for the educational value, but the
//   software will continue to work regardless.
// LCD drawing and plotting:
//   This is not implemented at all, since there is no display.
// LCD character, string, and number output:
//   Send ASCII characters through UART0 to a computer at 115,200
//   baud rate.  The automatic grader TExaS also uses this channel.
// Clock, Timer, Periodic Interrupts, and Delay:
//   These are unchanged, since they are part of the processor and
//   not the BoosterPack.
// Light and Temperature Sensors:
//   Simulate a sensor reading that takes a significant amount of
//   time before returning a result.  The light sensor takes about
//   0.8 seconds, and the temperature sensor takes about 1 second.
//   Attempting to end a measurement before the full amount of time
//   has passed will continue to return that the result is not ready
//   yet.  The input functions still block for the full duration
//   before returning.  Since there are no actual sensors, the
//   return value is arbitrary.

// What parts should you supply (BoM)?
// 1 solderless breadboard
// 6 potentiometers
// 1 1,000 ohm resistor 10%
// 1 female headphone jack
// 1 32 ohm headphones or ear buds
// 1 momentary switch
// 1 10,000 ohm resistor 10%
// many solid wires
...
void BSP_Button1_Init(void){
  ...
  GPIO_PORTF_DIR_R &= ~0x10;       // 5) make PF4 input
  GPIO_PORTF_AFSEL_R &= ~0x10;     // 6) disable alt funct on PF4
  GPIO_PORTF_PUR_R |= 0x10;        // enable pull-up on PF4
  GPIO_PORTF_DEN_R |= 0x10;        // 7) enable digital I/O on PF4
}
...
#define BUTTON1   (*((volatile uint32_t *)0x40025040))  /* PF4 */
...
void BSP_Button2_Init(void){
  ...
  GPIO_PORTF_AMSEL_R &= ~0x01;     // 3) disable analog on PF0
                                   // 4) configure PF0 as GPIO
  GPIO_PORTF_PCTL_R = (GPIO_PORTF_PCTL_R&0xFFFFFFF0)+0x00000000;
  GPIO_PORTF_DIR_R &= ~0x01;       // 5) make PF0 input
  GPIO_PORTF_AFSEL_R &= ~0x01;     // 6) disable alt funct on PF0
  GPIO_PORTF_PUR_R |= 0x01;        // enable pull-up on PF0
  GPIO_PORTF_DEN_R |= 0x01;        // 7) enable digital I/O on PF0
}
...
#define BUTTON2   (*((volatile uint32_t *)0x40025004))  /* PF0 */
...
void BSP_LCD_SetCursor(uint32_t newX, uint32_t newY){
  ...
  StY = newY;
// stand alone section
  // The goal of this adaptation is to change an LCD interface
  // with X- and Y-coordinates into a single character stream
  // in such a way that the text output looks good and is not
  // completely scrambled.  Due to the limitations of the UART,
  // X- and Y-coordinates, text color, background color, and
  // text size parameters must all be ignored.  This solution
  // assumes a particular order of operations:
  // 1) Initialize LCD
  // 2) Print strings with names of variables
  // 3) Move cursor to prepare to print a variable's value at a fixed position near its name
  // 4) Actually print the value
  // 5) Repeat Steps 3) and 4) for each variable
  // 6) Get new values
  // 7) Go to Step 3)
  // Instead of moving around the screen, this solution uses
  // carriage returns ('\r') to return to the first column of
  // the current line and over-write the previous values.
  // This should create a table.  The initial variable name
  // strings from Step 2) form the column headers in the table.
  // The very first time that this function is called it will
  // remember the desired newX and newY values.  That way, it
  // can return to the first column of the table by outputing
  // a carriage return whenever these values are observed.
  // Otherwise, this function simply outputs a space character
  // (' ') to separate the columns.
  // Some terminal configurations will treat carriage return
  // as new line ('\n'), returning the cursor to the first
  // column of the NEXT line.  This is fine as long as the
  // scrolling is readable.
  // IMPORTANT DIFFERENCE:
  // Print the values in Steps 3) and 4) in the same order as
  // the names were printed in Step 2) so everything matches.
  // This did not matter in the original LCD interface.
  static uint32_t firstCall = 1, x, y;
  if(firstCall == 1){
    firstCall = 0;
    x = newX;
    y = newY;
    BSP_LCD_DrawChar(newX*6, newY*10, '\r', StTextColor, ST7735_BLACK, 1);
    BSP_LCD_DrawChar(newX*6, newY*10, '\n', StTextColor, ST7735_BLACK, 1);
  } else{
    if((newX == x) && (newY == y)){
      BSP_LCD_DrawChar(newX*6, newY*10, '\r', StTextColor, ST7735_BLACK, 1); 
    } else{
      BSP_LCD_DrawChar(newX*6, newY*10, ' ', StTextColor, ST7735_BLACK, 1);
    }
  }
// end of stand alone section
}
...
/*  data1 = ((data1 - Ymin)*100)/Yrange;
  ...
  BSP_LCD_DrawPixel(TimeIndex + 12, 116 - data1, color1);
  BSP_LCD_DrawPixel(TimeIndex + 12, 115 - data1, color1);*/
}
...
// stand alone section
  if((SYSCTL_PRWTIMER_R&0x20) == 0){
    // turn timer on if not already on
    BSP_Time_Init();
  }
// end of stand alone section
}
...
uint32_t LightStart;               // system time when light sensor started (usec)
uint32_t LightCalls = 0;           // number of complete light measurements
#define LIGHTSAMPLETIME 800000     // number of 1 usec system time cycles needed to sample light sensor
...
// stand alone section
  LightStart = BSP_Time_Get();
  while((BSP_Time_Get() - LightStart) < LIGHTSAMPLETIME){};
  LightCalls = LightCalls + 1;
  light = LightCalls;              // arbitrary return value
// end of stand alone section
  LightBusy = 0;
  return light;
}
...
/*    if(LIGHTINT == 0x20){*/
// stand alone section
    if((BSP_Time_Get() - LightStart) < LIGHTSAMPLETIME){
// end of stand alone section
      return 0;                    // measurement needs more time to complete
    } else{
...
// stand alone section
      LightCalls = LightCalls + 1;
      lightLocal = LightCalls;     // arbitrary return value
// end of stand alone section
      *light = lightLocal;
      LightBusy = 0;
...
uint32_t TempStart;                // system time when temperature sensor started (usec)
uint32_t TempCalls = 0;            // number of complete temperature measurements
#define TEMPSAMPLETIME 1000000     // number of 1 usec system time cycles needed to sample temperature sensor
...
// stand alone section
  ...
  volt = TempCalls;                // arbitrary return value
  temp = TempCalls;                // arbitrary return value
// end of stand alone section
  *sensorV = volt;
  *localT = temp;
  TempBusy = 0;
